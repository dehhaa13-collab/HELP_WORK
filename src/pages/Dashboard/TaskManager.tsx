    ...
    <div className="task-manager">
      ...
      {sorted.length === 0 ? (
        <div className="task-empty">
          ...
          <p>{tasks.length === 0 ? 'Задач нет. Добавьте первую!' : 'Нет задач по этому фильтру.'}</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="task-content">
                  ...
                  <div className="task-meta">
                    <span className="task-priority-badge" style={{ color: task.done ? 'var(--color-text-muted)' : p.color, background: task.done ? 'var(--color-bg)' : p.bg }}>
                      {p.emoji} {p.label}
                    </span>
                    {clientName && (
                      <span className="task-client-badge">👤 {clientName}</span>
                    )}
                    {dl.text && (
                      <span className={`task-deadline-badge ${dl.isOverdue && !task.done ? 'overdue' : ''} ${dl.isSoon && !task.done ? 'soon' : ''}`}>
                        📅 {dl.text}
                      </span>
                    )}
                  </div>
